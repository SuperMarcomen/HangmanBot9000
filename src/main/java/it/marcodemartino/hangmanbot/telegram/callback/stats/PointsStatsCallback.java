...
public class PointsStatsCallback implements CallbackDataHandler {

    private final Bot bot;
    private final UserStatsService userStatsService;

    public PointsStatsCallback(Bot bot, UserStatsService userStatsService) {
        this.bot = bot;
    ...
    public void onCallbackData(CallbackQuery callbackQuery, String name, String args) throws Throwable {
        ...
        if (!callbackData.equals("stats") && !callbackData.equals("stats_points")) return;
    ...
    private InlineKeyboardMarkup generateKeyboard(long userId) {
        return new InlineKeyboardMarkup(new InlineKeyboardButton[][] {
                {
                        new PointsButton(true, userId),
                        new RatioButton(false, userId)
                },
                {
    ...
    private String generateMessageTopPlayers(int limit, long userId) {
        ...
        List<UserInfo> stats = userStatsService.getTopPlayersPoints(limit);
        if (!statsContainsUser(userId, stats)) {
            message.append(getParameterizedStringStats("message_stats_your_points", userId, userStatsService.getUserInfoOf(userId)));
            message.append(System.lineSeparator());
        }

        for (UserInfo topPlayer : stats)  {
            message.append(getParameterizedStringStats("message_stats_points_line", userId, topPlayer));
            message.append(System.lineSeparator());
        }
